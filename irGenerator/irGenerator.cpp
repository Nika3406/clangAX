...
public:
    ...
    void generateAssignment(shared_ptr<ASTNode> node) {
        ...
        if (node->children[0]->type == NodeType::ARRAY_LITERAL) {
            auto arrayLitNode = node->children[0];

            if (arrayLitNode->children.empty()) {
                cerr << "Error: Empty array literal" << endl;
                return;
            }
            ...
            for (auto& child : arrayLitNode->children) {
                ...
                if (Constant* constVal = dyn_cast<Constant>(elemVal)) {
                    ...
                    cerr << "Error: Non-constant array element" << endl;
                    return;
                }
            }
            ...
            // Allocate and store
            AllocaInst* arrayAlloca = createEntryBlockAlloca(currentFunction, varName, arrayType);
            builder->CreateStore(arrayConstant, arrayAlloca);
            namedValues[varName] = arrayAlloca;
    ...
    void generatePrint(shared_ptr<ASTNode> node) {
        ...
        // Check if we're printing an identifier that's an array
        if (node->children[0]->type == NodeType::IDENTIFIER) {
            ...
            if (var) {
                ...
                if (ArrayType* arrayType = dyn_cast<ArrayType>(varType)) {
                    // Print array elements
                    Value* formatStrPtr = getStringPtr("[");
                    ...
                    for (uint64_t i = 0; i < size; i++) {
                        ...
                        Value* elemPtr = builder->CreateInBoundsGEP(arrayType, var, indices);
                        Value* elem = builder->CreateLoad(elemType, elemPtr);

                        // Print element
                        string elemFormat = elemType->isIntegerTy() ? "%d" : "%f";
                        Value* elemFormatPtr = getStringPtr(elemFormat);
                        builder->CreateCall(printfFunc, {elemFormatPtr, elem});

                        if (i < size - 1) {
                            Value* commaPtr = getStringPtr(", ");
                            builder->CreateCall(printfFunc, {commaPtr});
                        }
                    }

                    Value* closeBracket = getStringPtr("]\n");
                    builder->CreateCall(printfFunc, {closeBracket});
                    return;
                }
            }
        }
    ...
    Value* generateArrayAccess(shared_ptr<ASTNode> node) {
        ...
        AllocaInst* arrayAlloca = namedValues[arrayName];
        if (!arrayAlloca) {
            cerr << "Error: Unknown array: " << arrayName << endl;
            return ConstantInt::get(*context, APInt(32, 0, true));
        }
        ...
        // Not an array
        cerr << "Error: Variable " << arrayName << " is not an array" << endl;
        return ConstantInt::get(*context, APInt(32, 0, true));
    }
        ...
        while (peek() != quote && peek() != '\0') {
            if (peek() == '\\') {
                ...
                switch (escaped) {
                    ...
                    case '\'': str += '\''; break;  // single quote
                    case '0':  str += '\0'; break;  // null
                    default:   str += escaped; break;  // unknown, keep as-is
                }
            } else {
                str += advance();
            }
